...
class ScdcTx:
    ...
    def write_scdc(self, base: int, count: int, data: bytearray) -> int:

        result = TSIX_TS_SetConfigItem(self.device.get_handle(), TSI_HDTX_SCDC_OFFSET_RW, base)
        if result < TSI_SUCCESS:
            return result
        TSIX_TS_SetConfigItem(self.device, TSI_HDTX_SCDC_DATA_RW, data, c_byte, count)

    def read_scdc(self, base: int, count: int) -> bytearray:

        result = TSIX_TS_SetConfigItem(self.device.get_handle(), TSI_HDTX_SCDC_OFFSET_RW, base)
        if result < TSI_SUCCESS:
            return bytearray()

        result = TSIX_TS_GetConfigItem(self.device.get_handle(), TSI_HDTX_SCDC_DATA_RW, c_byte, count)
    ...
    @staticmethod
    def save_scdc(base_address_range_1: int, size_range_1: int, data_range_1: bytearray,
