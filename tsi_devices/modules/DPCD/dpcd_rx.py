...
class DpcdRx:
    ...
    def write_dpcd(self, base: int, count: int, data: bytearray) -> int:

        result = TSIX_TS_SetConfigItem(self.device.get_handle(), TSI_W_DPRX_DPCD_BASE, base)
        if result < TSI_SUCCESS:
            return result
        result = TSIX_TS_SetConfigItem(self.device, TSI_DPRX_DPCD_DATA, data, c_byte, count)
        return result
    ...
    def read_dpcd(self, base: int, count: int) -> bytearray:
        ...
        result = TSIX_TS_GetConfigItem(self.device.get_handle(), TSI_DPRX_DPCD_DATA, c_byte, count)
